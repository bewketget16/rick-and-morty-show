      ...
      <div class="container mx-auto mt-8 flex-grow">
        <div class="container mx-auto mt-8" v-if="location || loading || error">
          <div class="max-w-3xl mx-auto" v-if="location">
            <div>
              <h2 class="text-2xl font-bold text-white">{{ location.name }}</h2>
              <p class="text-lg text-gray-300">
                Type: {{ location.type }} - Dimension: {{ location.dimension }}
              </p>
              <p class="text-lg text-gray-300">Created: {{ location.created }}</p>
            </div>
            <div class="border-t border-gray-300 mt-4 pt-4">
              <h3 class="text-xl font-bold mb-2 text-white">Residents</h3>
              <ul
              ...
              >
                ...
                >
                  <div class="flex items-center">
                    ...
                    />
                    <div>
                      <h4 class="text-lg font-bold text-gray-400">{{ resident.name }}</h4>
                      <p class="text-md text-gray-300">Status: {{ resident.status }}</p>
                      <p class="text-md text-gray-300">Species: {{ resident.species }}</p>
                      <p class="text-md text-gray-300">Gender: {{ resident.gender }}</p>
                    </div>
                  </div>
                </li>
              </ul>
